...
export class ObjectManager {
  ...
  makeBowl(){const g=new THREE.Group();const lift=this.effects.platformPause>0?.42:0;g.position.y=lift;
    ...
    g.userData={food:1,full:1,foodMesh:food,label:'Food bowl',type:'bowl',liftY:lift,garyEat:rate,raised:lift>0};shadow(g);return g}
  ...
  consume(bowl,amount,kind,stats){const used=Math.min(bowl.userData.food,amount);bowl.userData.food-=used;bowl.userData.foodMesh.scale.y=Math.max(.03,bowl.userData.food);bowl.userData.foodMesh.position.y=bowl.userData.liftY+.195+.045*bowl.userData.food;if(kind==='cat')stats.catFood+=used;else stats.stolen+=used;return used}
