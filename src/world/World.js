...
export class World {
  constructor(scene){
    this.scene=scene; this.interactables=[]; this.fireflies=[]; this.foliage=[]; this.fireflyBoost=1; this.floodlight=null; this.human=null;
  ...
  buildFloodlight(){if(this.floodlight)return this.floodlight;const g=new THREE.Group();const head=new THREE.Mesh(new THREE.BoxGeometry(.5,.3,.5),mat(0x56615d,.4,.45));head.position.set(0,4.15,-10.1);g.add(head);const bulb=new THREE.Mesh(new THREE.SphereGeometry(.09,8,6),new THREE.MeshBasicMaterial({color:0xfff0c0}));bulb.position.set(0,4.05,-10.15);g.add(bulb);const spot=new THREE.SpotLight(0xfff0c0,0,24,Math.PI/5,.5,1);spot.position.set(0,4.2,-10);const tg=new THREE.Object3D();tg.position.set(0,0,3);spot.target=tg;g.add(tg);g.add(spot);const beam=new THREE.Mesh(new THREE.ConeGeometry(3,13,24,1,true),new THREE.MeshBasicMaterial({color:0xfff0c0,transparent:true,opacity:0,depthWrite:false,side:THREE.DoubleSide}));beam.position.set(0,2.3,1);beam.rotation.x=-Math.PI/2;g.add(beam);this.scene.add(g);this.floodlight={group:g,spot,beam,on:()=>{spot.intensity=9;beam.material.opacity=.16},off:()=>{spot.intensity=0;beam.material.opacity=0}};return this.floodlight}
  buildHuman(){if(this.human)return this.human;const g=new THREE.Group(),mk=(w,h,d,c,r=.9)=>{const m=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),mat(c,r));m.castShadow=true;return m};const torso=mk(1,1.4,.6,0x725a48);torso.position.y=1.5;g.add(torso);const head=mk(.5,.5,.5,0xc9a984);head.position.y=2.45;g.add(head);for(const s of [-1,1]){const leg=mk(.34,1.4,.34,0x3a332c);leg.position.set(s*.3,.7,0);g.add(leg)}for(const s of [-1,1]){const arm=mk(.22,1.15,.22,0x5e4f40);arm.position.set(s*.72,1.5,0);arm.rotation.z=s*.06;g.add(arm)}g.position.set(0,0,-9.5);g.visible=false;this.scene.add(g);this.human={group:g,hAnchor:new THREE.Vector3(0,0,-6.6),shown:0,show:(intro,trust)=>{g.visible=true;this.human.shown=1},advance:(t)=>{g.position.z=-9.5+Math.min(1,t/2.4)*2.5;g.position.y=Math.abs(Math.sin(t*9))*.04},hide:()=>{g.visible=false;this.human.shown=0}};return this.human}
  update(t,nightness){for(const f of this.foliage){const s=f.userData.surge||0;f.rotation.z=Math.sin(t*.55+f.userData.phase)*.008+(s?Math.sin(t*22)*.16*s:0);f.rotation.x=Math.cos(t*.42+f.userData.phase)*.005+(s?Math.cos(t*18)*.08*s:0);if(s)f.userData.surge=Math.max(0,s-.018)}if(this.fireflyPoints){this.fireflyPoints.material.opacity=Math.min(1,(.18+nightness*.75)*(this.fireflyBoost||1));const a=this.fireflyPoints.geometry.attributes.position;for(let i=0;i<a.count;i++)a.array[i*3+1]+=Math.sin(t*1.5+i)*.0008;a.needsUpdate=true}if(this.eyes.visible)this.eyes.scale.setScalar(.75+Math.sin(t*2.5)*.2)}
}
