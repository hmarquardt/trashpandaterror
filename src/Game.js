...
export class Game {
  constructor(container){
    ...
    this.selected=null;this.dragging=null;this.hosing=false;this.hoseTimer=0;this.baseTrust=this.averageTrust();this.raidTimer=null;this.raidsTotal=1;this.raidsDone=0;this.raidSeq=0;this.theftNotified=false;this.camPulse=0;this.beatT=0;this.lightFlashed=false;
  ...
  context(){const g=this.gary,L=this.nightLog,self=this;
    return {objects:this.objects,stats:this.stats,effects:this.prog.effects(),night:this.prog.night,
      ...
      onGarySeen:gg=>{this.world.eyes.visible=false;this.world.rustleBush(gg.root.position.x,gg.root.position.z);this.audio.play('rustle',panOf(gg.root.position.x));this.ui.toast('GARY','Gary has entered the yard. He looks confident.');this.ui.showThought(gg,'food?',this.camera);this.camPulse=Math.max(this.camPulse,.5);this.alarm()},
  ...
  hose(dt){if(!this.hosing||!this.hoseTarget||this.state.phase!=='NIGHT')return;this.hoseTimer-=dt;if(this.hoseTimer<=0){this.hoseTimer=.045;const origin=new THREE.Vector3(5,.35,-7),fx=this.prog.effects();this.objects.sprayHose(origin,this.hoseTarget);for(const a of [...this.cats,this.gary])if(a.root.visible&&a.root.position.distanceTo(this.hoseTarget)<fx.hoseRange){const pan=panOf(a.root.position.x);if(a.type==='raccoon'){a.memory.hose+=1;this.stats.adaptation+=1;this.nightLog.hoses++;this.camPulse=Math.max(this.camPulse,.25);this.audio.play('splash',pan)}else this.stats.catStartles=(this.stats.catStartles||0)+1;a.fleeFrom(this.hoseTarget,.9*fx.hoseForce,Math.max(1,fx.hoseForce));this.context().onSprayed(a,true)}}}
