...
import { NIGHT, RAIDS, REWARD } from './systems/Tuning.js';
...
export class Game {
  constructor(container){
    ...
    this.selected=null;this.dragging=null;this.hosing=false;this.hoseTimer=0;this.baseTrust=this.averageTrust();this.raidTimer=null;this.raidsTotal=1;this.raidsDone=0;this.raidSeq=0;this.theftNotified=false;this.camPulse=0;this.alarmTimer=0;
    this.nightLog=this.freshNightLog();
    this.setupLights();this.addStars();this.placeDefaults();this.bind();this.animate();
    this.audio=new AudioManager();
    this.ui.showShop(this.prog,this.gary.traits,this.lastNight={raids:0});
  }
  freshNightLog(){return {probes:0,bumps:0,reroutes:0,flanks:0,climbs:0,raids:0,hoses:0,munchCats:0,catsFed:[],catStartles:{}}}
  ...
  placeDefaults(){this.objects.place('bowl',new THREE.Vector3(-1.2,0,-6.8));this.objects.place('bowl',new THREE.Vector3(1.2,0,-6.8));this.objects.place('barrier',new THREE.Vector3(-4,0,-4.4));const s=this.objects.place('sprinkler',new THREE.Vector3(-4.2,0,-2.8));s.rotation.y=.4}
  resetNight(){
    ...
    this.raidsTotal=Math.min(4,Math.max(1,RAIDS.perNight(this.prog.night)+(this.gary.personality.raidsBias||0)));this.raidsDone=0;this.hoseTimer=0;this.theftNotified=false;this.camPulse=0;
    this.nightLog=this.freshNightLog();
  ...
  persistSave(){persistSave({cats:{tom:this.cats[0].def.trust,princess:this.cats[1].def.trust,nero:this.cats[2].def.trust},garyMemory:this.gary.memory,garyPersonality:this.gary.traits,prog:{chow:this.prog.chow,night:this.prog.night,upgrades:this.prog.upgrades,bestRating:this.prog.bestRating}})}
  ...
  context(){const g=this.gary,L=this.nightLog,self=this;
    return {objects:this.objects,stats:this.stats,effects:this.prog.effects(),night:this.prog.night,
      ...
      onMunch:c=>this.audio.play('munch'),
      onGarySeen:gg=>{this.world.eyes.visible=false;this.audio.play('rustle');this.ui.toast('GARY','Gary has entered the yard. He looks confident.');this.ui.showThought(gg,'food?',this.camera);this.camPulse=Math.max(this.camPulse,.5);this.alarm()},
      onBarrier:gg=>{L.bumps++;this.audio.play('bump');this.objects.bumpDust(gg.root.position);this.ui.showThought(gg,'dang fence',this.camera)},
      onReroute:gg=>{L.reroutes++;this.ui.showThought(gg,'around it is',this.camera)},
      onProbe:gg=>{L.probes++;this.audio.play('deter');this.ui.showThought(gg,'got a bead on this…',this.camera)},
      onCover:gg=>{L.flanks++;this.audio.play('rustle');this.ui.showThought(gg,'cover',this.camera)},
      onClimb:gg=>{L.climbs++;this.audio.play('climb');this.ui.showThought(gg,'climbing…',this.camera);this.ui.toast('GARY CLIMBS','He hauled himself up onto the feeding deck.')},
      ...
      onSprayed:(a,manual)=>{this.audio.play('spray');if(a.type==='cat'){const loss=(1-a.def.noiseTolerance)*4;a.def.trust-=loss;this.stats.catStartles=(this.stats.catStartles||0)+1;L.catStartles[a.def.id]=(L.catStartles[a.def.id]||0)+1;this.ui.showThought(a,'!!',this.camera)}else this.ui.showThought(a,manual?'RUDE':'noted.',this.camera)},
  ...
  hose(dt){if(!this.hosing||!this.hoseTarget||this.state.phase!=='NIGHT')return;this.hoseTimer-=dt;if(this.hoseTimer<=0){this.hoseTimer=.045;const origin=new THREE.Vector3(5,.35,-7),fx=this.prog.effects();this.objects.sprayHose(origin,this.hoseTarget);for(const a of [...this.cats,this.gary])if(a.root.visible&&a.root.position.distanceTo(this.hoseTarget)<fx.hoseRange){if(a.type==='raccoon'){a.memory.hose+=1;this.stats.adaptation+=1;this.nightLog.hoses++;this.camPulse=Math.max(this.camPulse,.25)}else this.stats.catStartles=(this.stats.catStartles||0)+1;a.fleeFrom(this.hoseTarget,.9*fx.hoseForce);this.context().onSprayed(a,true)}}}
  deriveObservations(){const L=this.nightLog,out=[];const sideOf=x=>x<0?'west':'east';
    ...
    const seen=this.nightLog.probes+this.nightLog.flanks+this.nightLog.climbs+this.nightLog.bumps+this.nightLog.reroutes;
