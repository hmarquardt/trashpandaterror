...
const MOODS={clear:{bg:[.025,.035,.045],li:1,fog:.022,fire:.6,warm:0},warm:{bg:[.04,.028,.02],li:1.06,fog:.028,fire:1,warm:.14},firefly:{bg:[.032,.05,.05],li:1.02,fog:.026,fire:2.6,warm:.04},hazy:{bg:[.035,.045,.055],li:.95,fog:.04,fire:.5,warm:.02},storm:{bg:[.02,.04,.055],li:.9,fog:.036,fire:.4,warm:0,storm:1},neighbor:{bg:[.028,.05,.06],li:1,fog:.024,fire:1,warm:.05}};
...
export class Game {
  ...
  context(){const g=this.gary,L=this.nightLog,self=this;
    return {objects:this.objects,stats:this.stats,effects:this.prog.effects(),night:this.prog.night,
      ...
      onSprayed:(a,manual)=>{this.audio.play('spray');if(a.type==='cat'){const now=this.state.elapsed;this.ui.showThought(a,'!!',this.camera);if(now-(a.lastStartle??-99)<1.5)return;const loss=(1-a.def.noiseTolerance)*4;a.def.trust-=loss;a.lastStartle=now;this.stats.catStartles=(this.stats.catStartles||0)+1;L.catStartles[a.def.id]=(L.catStartles[a.def.id]||0)+1}else{L.contested=true;this.ui.showThought(a,manual?'RUDE':'noted.',this.camera)}},
  ...
  finish(){if(this.state.phase==='RESULTS')return;this.state.phase='RESULTS';this.finishedThisNight=true;this.audio.stop();this.world.eyes.visible=false;clearTimeout(this.raidTimer);this.gary.active=false;this.gary.root.visible=false;this.gary.state='gone';const h=this.history;h.sprinkler.total+=this.nightLog.probes;h.sprinkler.streak=this.nightLog.probes>0?h.sprinkler.streak+1:0;h.side.east+=this.nightLog.sideE;h.side.west+=this.nightLog.sideW;h.climbs.total+=this.nightLog.climbs;h.climbs.streak=this.nightLog.climbs>0?h.climbs.streak+1:0;if(this.stats.stolen>.6)h.bigTheft+=1;const endTrust=this.averageTrust(),delta=endTrust-this.baseTrust,served=this.objects.bowls.length;const rw=computeReward(this.prog.night,this.stats,this.cats,endTrust,delta,served);this.prog.addChow(rw.total);const foodTotal=Math.max(.001,this.stats.catFood+this.stats.stolen);const defense=Math.round((this.stats.catFood/foodTotal)*70+(this.cats.filter(c=>c.wasFed).length/3)*30);const rating=RATINGS.find(r=>defense>=r.min);this.prog.setBest(rating.grade);this.lastNight={raids:this.raidsDone,chowEarned:rw.total,recognition:this.recognitionLine(),breakdown:rw.breakdown};console.info('REWARD_BREAKDOWN '+JSON.stringify(rw.breakdown));const obs=this.deriveObservations();const story=genHeadline(this.prog.night,this.stats,this.cats,rw,{raids:this.raidsDone,climbs:this.nightLog.climbs});this.persistSave();this.audio.play('dawn');this.ui.finish({...this.stats,catFood:this.stats.catFood/served,stolen:this.stats.stolen/served},this.cats,delta,rw,this.prog,rating,story,obs)}
  ...
  checkSprinklers(ctx){for(const s of this.objects.sprinklers){if(s.userData.cooldown>0)continue;let catIn=false;for(const c of this.cats){if(this.state.elapsed-(c.lastSprayedAt??-99)<12)continue;if(c.root.visible&&c.root.position.distanceTo(s.position)<s.userData.radius){if(s.userData.catAware){catIn=true;s.userData.spare=.45;this.audio.play('safe');}else{c.lastSprayedAt=this.state.elapsed;this.objects.triggerSprinkler(s);c.rememberSprinkler(s);c.fleeFrom(s.position,1);ctx.onSprayed(c,false);this.ui.toast('FRIENDLY FIRE',`${c.def.name} was startled by the sprinkler.`);}break}}if(catIn)continue}}
